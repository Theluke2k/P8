...
% Two styled subplots: plume+robots and charger+robots using original robot style/colors
clear; close all; clc;
...
surf(ax1, Xg, Yg, Z_true, 'FaceColor','interp','EdgeColor','k','EdgeAlpha',0.1,'FaceAlpha',0.9);
contour3(ax1, Xg, Yg, Z_true, linspace(0,max(Z_true(:)),8),'k','LineWidth',0.5);
for m=1:M
...
colormap(ax1, parula); shading(ax1,'interp'); lighting(ax1,'gouraud'); material(ax1,'dull');
axis(ax1,[xmin xmax ymin ymax 0 4]); view(ax1,45,30);
xlabel(ax1,'X [m]'); ylabel(ax1,'Y [m]'); zlabel(ax1,'Plume Height');
title(ax1,'Initial Process and Robots'); grid(ax1,'on'); box(ax1,'on');

% --- Right: Charger + Robots ---
ax2 = subplot(1,2,2); hold(ax2,'on');
surf(ax2, Xg, Yg, Ch*5, 'FaceColor','interp','EdgeColor','k','EdgeAlpha',0.1,'FaceAlpha',0.7);
contour3(ax2, Xg, Yg, Ch*5, linspace(min(Ch(:))*5,max(Ch(:))*5,6),'k','LineWidth',0.5);
for m=1:M
...
colormap(ax2, autumn); shading(ax2,'interp'); lighting(ax2,'gouraud'); material(ax2,'dull');
axis(ax2,[xmin xmax ymin ymax 0 2]); view(ax2,45,30);
xlabel(ax2,'X [m]'); ylabel(ax2,'Y [m]'); zlabel(ax2,'Charge Rate');
title(ax2,'Charging Station and Robots'); grid(ax2,'on'); box(ax2,'on');
